...
export async function POST(request: NextRequest) {
  ...
  // Aggregate per-activity stats
  const stats: Record<string, { name: string; totalSeconds: number; sessions: number; goalHours?: number; goalSessions?: number }> = {};
  ...
  const statsText = Object.values(stats).map((s) => {
    ...
    let goal = '';
    if (s.goalHours) goal = ` (goal: ${s.goalHours}h/wk)`;
    else if (s.goalSessions) goal = ` (goal: ${s.goalSessions} sessions/wk)`;
