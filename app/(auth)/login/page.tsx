'use client'

import { createClient } from '@/lib/supabase/client'

const phrases = [
  "La disciplina es elegir entre lo que quieres ahora y lo que más quieres.",
  "El éxito no es un accidente, es trabajo duro cada día.",
  "Pequeños progresos diarios generan grandes resultados.",
  "La constancia supera al talento cuando el talento no trabaja.",
  "Hoy es otro día para ser mejor que ayer."
]

export default function LoginPage() {
  const supabase = createClient()
  const phrase = phrases[new Date().getDay() % phrases.length]

  const handleLogin = () => {
    supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback`
      }
    })
  }

  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        {/* Logo */}
        <div className="text-center mb-10">
          <h1 className="text-5xl font-bold text-[#f1f5f9] tracking-tight mb-2">APEX</h1>
          <p className="text-sm text-[#94a3b8]">Tu sistema de rendimiento personal</p>
        </div>

        {/* Motivational phrase */}
        <div className="bg-[#111111] border border-[#1f1f1f] rounded-xl p-5 mb-8">
          <p className="text-sm text-[#94a3b8] italic text-center leading-relaxed">&quot;{phrase}&quot;</p>
        </div>

        {/* Login button */}
        <button
          onClick={handleLogin}
          className="w-full flex items-center justify-center gap-3 bg-[#f1f5f9] hover:bg-white text-[#0a0a0a] font-semibold py-3.5 px-6 rounded-xl transition-all duration-200 text-sm"
        >
          <svg width="18" height="18" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          Continuar con Google
        </button>

        <p className="text-xs text-[#475569] text-center mt-6">
          Al continuar aceptas usar esta app para tu beneficio personal
        </p>
      </div>
    </div>
  )
}
